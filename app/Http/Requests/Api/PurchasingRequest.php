    ...
    public function rules(): array
    {
        ...
        return match ($resource) {
            ...
            'goods-receipts', 'goods-receipt-notes' => [
                ...
                'items.*.purchase_order_item_id' => [...$nullable, 'uuid', Rule::exists('purchase_order_items', 'id')],
                'items.*.product_id' => [...$required, 'uuid', Rule::exists('products', 'id')],
                'items.*.received_quantity' => ['sometimes', 'numeric', 'min:0'],
                'items.*.received_qty' => ['sometimes', 'numeric', 'min:0'],
                'items.*.rejected_quantity' => ['sometimes', 'numeric', 'min:0'],
                'items.*.rejected_qty' => ['sometimes', 'numeric', 'min:0'],
                'items.*.notes' => [...$nullable, 'string'],
            ],
            'goods-receipt-items', 'goods-receipt-note-items' => [
                'goods_receipt_note_id' => [...$required, 'uuid', Rule::exists('goods_receipt_notes', 'id')],
                'purchase_order_item_id' => [...$nullable, 'uuid', Rule::exists('purchase_order_items', 'id')],
                'product_id' => [...$required, 'uuid', Rule::exists('products', 'id')],
                'received_qty' => ['sometimes', 'numeric', 'min:0'],
                'rejected_qty' => ['sometimes', 'numeric', 'min:0'],
                'received_quantity' => ['sometimes', 'numeric', 'min:0'],
                'rejected_quantity' => ['sometimes', 'numeric', 'min:0'],
                'notes' => [...$nullable, 'string'],
            ],
            'purchase-requests' => [
